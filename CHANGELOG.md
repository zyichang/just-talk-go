...
- Added `silence_stop_ms`, which ends a recording automatically once a trailing window of that length holds almost no speech. It routes through the same stop path as the hotkey, so a session is still marked user-stopped and its final text is dispatched exactly once. Works with `enabled = false`, in which case the gate measures levels but forwards every frame untouched.
